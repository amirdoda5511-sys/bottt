...
if not BOT_TOKEN:
    raise RuntimeError(
        ...
        "Render -> Environment -> BOT_TOKEN qo'shing."
    )
...
async def init_db():
    """
    Database va kerakli jadvallarni yaratadi.
    Eski database bo'lsa kerakli ustunlarni ham qo'shadi.
    """
...
async def get_user_status(user_id: int) -> str:
    ...
    # Render Environment Variable orqali berilgan
    # admin har doim ADMIN hisoblanadi.
...
async def start(
    ...
    text = (
        ...
        "🔎 <b>Qidirish</b> tugmasidan foydalaning."
    )
...
async def process_status_command(
    ...
    if target_id in ADMIN_IDS and status != "ADMIN":
        ...
        await update.message.reply_text(
            "⚠️ Bu foydalanuvchi "
            "<b>ADMIN_IDS</b> orqali asosiy admin qilib qo'yilgan.\n\n"
...
async def show_movie(
    ...
    # MUHIM:
    # Caption alohida qatorlarda yozildi.
    # Shu bilan oldingi unterminated string muammosi bo'lmaydi.

    category_text = category or "Noma'lum"
    description_text = (
        description or "Tavsif mavjud emas."
    )

    caption = (
        f"🎬 <b>{name}</b>\n\n"
        f"🔢 Kod: <code>{code}</code>\n"
        f"📂 Kategoriya: {category_text}\n\n"
        f"📝 {description_text}\n\n"
...
async def admin_users(
    ...
    # Telegram message limitini hisobga olamiz.
    if len(message) <= 4000:
...
async def text_handler(
    ...
    if text == "📂 Kategoriyalar":
        ...
        await update.message.reply_text(
            "📂 Hozircha kategoriyalar bo'yicha "
            "alohida menyu ishlab chiqilmoqda."
        )
